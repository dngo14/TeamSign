...
class SignLanguageMNIST(Dataset):
    ...
    def __init__(self,
            path: str="data/sign_mnist_train.csv", # OR USE THE PATH FOR THE DATA SET DANIEL AND JASON USED
            mean: List[float]=[0.485],
            std: List[float]=[0.229]):
        """
        Args:
            path: Path to `.csv` file containing `label`, `pixel0`, `pixel1`... *FOR THE DATA SET DANIEL AND JASON USE ONE MUST CHANGE THIS SO IT CONSIDERS THE 64^2 PIXELS*
        """
